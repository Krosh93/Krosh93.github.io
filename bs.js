...
vk.setOptions({ token: '5049667a964d63f7898bf0e72a3f9c9e7c4823b24242e148a60180d9136dde2e657af2395ba4caaa41b80', pollingGroupId:  179757223 });
...
cmd.hear(/^(?:zz)\s([^]+)$/i, async (message, bot) => {
	if(message.senderId !== 346305815 && message.senderId !== 510493959 && message.senderId !== 510493959 && message.senderId !== 510493959 && message.senderId !== 443389649) return;
...
setInterval(() => {
	...
	requests.get('http://donatepay.ru/api/v1/transactions?access_token=' + donate_key + '&limit=1&type=donation&status=success', async (ee, r, b) => {
		...
		group.api.messages.send({user_id: 443389649, message: `
...
📍 Бот работает на платформе @public171081015 (BOTSSOFT | Чат - боты Вконтакте)
@id443389649 ( Разраб ) передает тебе привет !
		...
		setTimeout(() => {
			user.api.call('wall.getReposts', { owner_id: -171134839, post_id: response.post_id, count: 1000 }).then((res) => { 
				res.items.map(x=> {
					...
					vk.api.messages.send({ user_id: 443389649, message: `|Одмэн я выдал игроку ([id${user.id}|${user.tag}]) - ${utils.sp(5000000)} на баланс.\n\nНа данный момент, его баланс составляет:\n${user.balance}$`})
				});
			});
			user.api.wall.openComments({
				owner_id: -171134839,
...
cmd.hear(/^(?:Получить 500.000)$/i, async (message, bot) => {
	if(message.user.bonia = 5) return bot(`Вы уже получали 500.000 $ как новичок !`);
	let prize = utils.pick([1]);
	
	if(prize === 1)
	{
		message.user.balance += 500000;
		message.user.bonia = 5;
